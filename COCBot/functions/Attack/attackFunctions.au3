...
; Description ...: Contains misc functions used by attack profiles
; Syntax ........:
; Parameters ....:
; Return values .: None
...
; Used to calculate the angle between two points, because we are using X, Y differences its always a right angle triangle
Func calculateAngleBetweenPoints($point1, $point2)
	Local $result = 0
	Local $differenceX = Abs($point1[0] - $point2[0])
	Local $differenceY = Abs($point1[1] - $point2[1])
	
	; Value calculated is in Radians, if you want to know the degrees use _Degree($result) on the returned value
	$result = ATan($differenceY / $differenceX)
