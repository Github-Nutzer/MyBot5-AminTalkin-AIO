...
; Description ...: Contains functions for eight finger implosion deployment
; Syntax ........:
; Parameters ....:
; Return values .: None
...
Func eightFingerImplosionMulti($dropAmount, $slotsPerEdge = 0)
	...
	For $i = 0 To $slotsPerEdge - 1
		...
		implosionSideDrop($TopLeft, $sideTopLeft, $i + 1, $slotsPerEdge, $troopsPerSlot)
		implosionSideDrop($TopRight, $sideTopRight, $i + 1, $slotsPerEdge, $troopsPerSlot)
		implosionSideDrop($BottomRight, $sideBottomRight, $i + 1, $slotsPerEdge, $troopsPerSlot)
		implosionSideDrop($BottomLeft, $sideBottomLeft, $i + 1, $slotsPerEdge, $troopsPerSlot, True)

		$troopsLeft -= ($troopsLeft < $troopsPerSlot) ? $troopsLeft : $troopsPerSlot
	Next
EndFunc   ;==>eightFingerImplosionMulti

Func eightFingerImplosionDropOnEdge($kind, $dropAmount, $position = 0)
		...
		Case Else
			Switch $troopsPerEdge
				...
				Case 2
					multiDouble($troopsPerEdge)
				Case Else
					eightFingerImplosionMulti($troopsPerEdge, $position)
			EndSwitch
	EndSwitch
EndFunc   ;==>eightFingerImplosionDropOnEdge