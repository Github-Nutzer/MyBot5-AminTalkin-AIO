...
Func determineSideFromPoints($pointsArray)
	Local $return = 0, $maxValue = 0, $sidePoints = 0
...
Func customDeployMulti($sideCoords, $side, $dropAmount, $slotsPerEdge = 0)
	...
	For $i = 0 To $slotsPerEdge - 1
		...
		blossomSideDrop($sideCoords, $side, $i + 1, $slotsPerEdge, $troopsPerSlot, True)

		$troopsLeft -= ($troopsLeft < $troopsPerSlot) ? $troopsLeft : $troopsPerSlot
	Next
EndFunc   ;==>customDeployMulti

Func customDeployDropOnEdge($kind, $sideCoords, $side, $dropAmount, $position = 0)
		...
		Case Else
			Switch $troopsPerEdge
				...
				Case 2
					sideDouble($sideCoords, $troopsPerEdge)
				Case Else
					customDeployMulti($sideCoords, $side, $troopsPerEdge, $position)
			EndSwitch
	EndSwitch
EndFunc   ;==>customDeployDropOnEdge

Func customDeployDropOnEdges($deploySide, $side, $kind, $dropAmount, $position = 0)
	KeepClicks()
	customDeployDropOnEdge($kind, $deploySide, $side, $dropAmount, $position)			
	ReleaseClicks()
						...
						If $dropAmount > 0 Then
							Switch $positionSide
								...
								Case Else
									...
									If customDeployDropOnEdges($deploySide, $side, $barPosition, $dropAmount, $position) Then
										If _Sleep(SetSleep(1)) Then Return
									EndIf
							EndSwitch
						EndIf
